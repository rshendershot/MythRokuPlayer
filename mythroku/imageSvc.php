...
if (isset ($_GET['image'])) { //send a file spec
	...
	if($chanid && $starttime) {		
		...
		$tfile = $record->storagegroups->dirname . $record->basename . '.1.png'; 
    	if(!file_exists($file)) { //generate preview images since the user may not be invoking this from myth frontend
    		...
    		file_get_contents($MythContentSvc . 'GetPreviewImage' . "?ChanId=$chanid&StartTime=$rawstarttime&SecsIn=1");
    		rename( $tfile, $file );
//     		error_log(
//     			"*** " . implode( '|',
// 			    	get_headers(
